(() => {
  ...
  const animate=now=>{const t=(now-start)/1000;overlay.querySelectorAll('.traffic-vehicle').forEach((dot,index)=>{const p=paths[index%paths.length];const len=overlay.querySelectorAll('path')[index%Math.max(1,overlay.querySelectorAll('path').length)];if(!len)return;const point=len.getPointAtLength((t*(p.level==='SEVERE'?18:p.level==='HIGH'?28:42)+index*75)%len.getTotalLength());dot.setAttribute('cx',point.x);dot.setAttribute('cy',point.y);});raf=requestAnimationFrame(animate);};
